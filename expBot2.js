...
class Memory
{
    ...
    {
        if(drawStreak > dynamiteValue -1.5)
        {
    ...
    makeMove(gamestate) {
        ...
        var pD = 0;
            if( this.currentDrawStreak > numberOfDrawsDynOneThird)
            {
                ...
                pD = Math.max(0, pD * Math.pow((values.dynProb ),0.5));
                pW = Math.max(0, pW * Math.pow((values.watProb ),0.5));
                pRps = Math.max(0, pRps * Math.pow((values.rpsProb ),0.5));

                if(this.drawStreak > this.ev - 0.5)
                {
                    pD = Math.max(0, pD * Math.pow((values.dynProb ),2));
                    pW = Math.max(0, pW * Math.pow((values.watProb ),2));
                    pRps = Math.max(0, pRps *  Math.pow((values.rpsProb ),2));
